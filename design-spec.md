...
- **Hero**: as duas mãos (robô + humana) recortadas do vídeo em PNG com transparência (`mao-robo.png`, `mao-humana.png`), com a **posição controlada pela rolagem**.
  ...
  - **Movimento**: `--afasta` (0 = composição original da foto) entra num `translateX(calc(var(--afasta) * ±58%))`. Ao ABRIR a página as mãos já estão na tela e perto (`--afasta` = 0.28) — o Igor pediu isso explicitamente. O encontro se completa na primeira metade da rolagem (easeOutCubic, `p/0.5`), com a humana fechando 12% mais rápido pra dar profundidade. A opacidade cai de 0.12 a 0 entre 8% e 70% da rolagem, então elas somem antes do bloco 2 entrar.
  - **Reversível de graça**: a posição é função pura do scroll, então rolar pra cima desfaz o movimento sem código extra.
  - Opacidade base 0.12, P&B. Sem JS: composição final, parada.
  - **Palco escalona com a tela** (124% desktop / 150% ate 1024px / 200% mobile). NAO travar em px: antes estava `min(1360px, 116%)` e num monitor 1920 o palco virava so 71% da largura, deixando as maos flutuando no meio com margens vazias.
  - **prefers-reduced-motion NAO desliga as animacoes** (decisao do Igor, 2026-08-11): o PC dele tinha 'efeitos de animacao' desligado no Windows e o site ficava totalmente estatico. Hoje o media query so remove o DESLOCAMENTO dos reveals (vira fade puro); as maos, a conversa do agente e os cards continuam animando.
